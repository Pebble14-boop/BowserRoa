...
if attack == AT_DSPECIAL_2{
	if window == 1{
		if window_timer == 1{
			//this specifically allows what
			set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_GOTO, 2);
			set_attack_value(AT_DSPECIAL_2, AG_NUM_WINDOWS, 7);
		}
		//this here is just what happens when you hold a direction
		if (spr_dir == 1 && left_down == true) || (spr_dir == -1 && right_down == true){
			set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_GOTO, 5);
		}
		else{
			//change this window number and it changes which attack it goes to.
			//this is also what happens when nothing is pressed.
			set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_GOTO, 2);
		}
	}
	
	if window == 2{
		//basically, makes it so fthrow only displays the frames it needs.
		set_attack_value(AT_DSPECIAL_2, AG_NUM_WINDOWS, 4);
	}
	
	if window == 5{
		//don't necessarily know what this does
		//if the num_windows number is changed,the enemyV falls out of this move
		set_attack_value(AT_DSPECIAL_2, AG_NUM_WINDOWS, 7);
	}
